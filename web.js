...
(function(){
  ...
  function openExternalUrl(url, state){
    ...
    // V20-19: 웹사이트와 순례길 모두 외부 복귀는 브라우저의 기본 복원에 맡긴다.
  ...
  function restoreIntegratedState(){
    ...
    if(state.module === 'web'){
      // V20-19: 웹사이트 화면은 브라우저가 돌아온 화면을 그대로 복원하게 둔다.
      // 여기서 openWebView/renderWebList를 다시 호출하면 복귀 순간 배경 지도와 목록이 겹쳐 보인다.
      return;
    }

    if(state.module === 'trail'){
      // V20-19: 순례길도 외부 복귀 시 강제 재오픈/지도 재생성을 하지 않는다.
      return;
    }
  }

  window.addEventListener('pageshow', function(){
    // V20-19: 외부사이트 복귀 시 순례길 지도 relayout을 강제로 반복하지 않는다.
  ...
  function renderWebList(){
    ...
    filtered.forEach(s => {
      ...
      card.addEventListener('click', function(e){
        ...
        // V20-19: 교구 카드도 저장/복원 없이 즉시 이동한다.
        if(isDioceseCard){
