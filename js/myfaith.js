...
(function(){
  window.bindMyFaithLifePanel = function(on){
    ...
    function resetCoverBackAfterMyFaith(reason){
      ...
      /* V8-1-14-20-MYFAITH-QUIET-CLOSE: 나의신앙생활 닫힘 자체는 커버 종료문구 상황이 아니므로 강제 종료문구를 준비하지 않는다. */
    ...
    function quietCoverAfterMyFaithClose(reason){
      try{
        reason = reason || 'my-faith-local-close';
        clearGenericCoverToastFlag();
        clearMyFaithExternalLinkFlag();
        try{ sessionStorage.removeItem('oai_force_next_cover_back_toast_until'); sessionStorage.removeItem('oai_force_next_cover_back_toast_reason'); }catch(_e){}
        try{ if(typeof window._oaiSuppressNextCoverBackToast === 'function') window._oaiSuppressNextCoverBackToast(900, reason); }catch(_e){}
        ...
        setTimeout(function(){
          ...
          try{ if(typeof window._clearCoverExitArmed === 'function') window._clearCoverExitArmed(); }catch(_e){}
          try{ var oldToast=document.getElementById('_bt'); if(oldToast && oldToast.parentNode) oldToast.parentNode.removeChild(oldToast); }catch(_e){}
          try{ if(typeof window._resetCoverBackTrap === 'function') window._resetCoverBackTrap(reason); else if(typeof window._oaiArmCoverBackTrap === 'function') window._oaiArmCoverBackTrap(reason, {force:true}); }catch(_e){}
        }, 80);
        return true;
      }catch(e){ console.warn('[가톨릭길동무]', e); return false; }
    }
    ...
    function closeModal(){
      ...
      if(!(fromExternal || pendingCoverToast)){
        ...
        quietCoverAfterMyFaithClose('my-faith-local-close');
        return;
      }
      try{ clearMyFaithExternalLinkFlag(); }catch(_e){}
      /* 외부 사이트 복귀처럼 실제 페이지/외부 흐름이 섞인 경우에만 기존 커버 복귀 정리를 사용한다. */
    ...
    function markMyFaithExternalLink(){
      /* V8-1-14-20-MYFAITH-QUIET-CLOSE: 나의 신앙생활 외부링크는 가톨릭 웹사이트와 같은 공통 외부 이동 흐름을 사용한다.
