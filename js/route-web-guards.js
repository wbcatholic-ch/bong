...
(function(){
  ...
  function openNewTab(url){ if(!url) return; try{ if(typeof window.markExternalReturnStabilize === 'function') window.markExternalReturnStabilize('route-new-tab'); }catch(_e){} try{ var w=window.open(url,'_blank','noopener'); if(w) return; }catch(e){ console.warn("[가톨릭길동무]", e); } try{ var a=document.createElement('a'); a.href=url; a.target='_blank'; a.rel='noopener'; document.body.appendChild(a); a.click(); setTimeout(function(){try{a.remove();}catch(e){ console.warn("[가톨릭길동무]", e); }},300); }catch(e){ try{ if(typeof window.oaiClearExternalNavigationState === 'function') window.oaiClearExternalNavigationState(); }catch(_e){} alert('새창 열기가 차단되었습니다. 브라우저의 팝업 허용을 확인해 주세요.'); } }
