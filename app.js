...
function _closeShrineVisitCardsModal(){
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V6-43';
...
const _PARISH_ASSET_VERSION='V6-43';
...
const _PRAYER_ASSET_VERSION='V6-43';
...
const _RETREAT_ASSET_VERSION='V6-43';
...
const _SHRINE_ASSET_VERSION='V6-43';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function renderList(){
  ...
  entries.forEach(({s,i})=>{
   ...
   if(pilgrimRegister){
    let touchMoved=false, sx=0, sy=0, lastOpenAt=0;
    pilgrimRegister.addEventListener('touchstart', function(e){
      const t=e.touches&&e.touches[0];
      touchMoved=false;
      if(t){ sx=t.clientX; sy=t.clientY; }
    }, {passive:true});
    pilgrimRegister.addEventListener('touchmove', function(e){
      const t=e.touches&&e.touches[0];
      if(t && (Math.abs(t.clientX-sx)>10 || Math.abs(t.clientY-sy)>10)) touchMoved=true;
    }, {passive:true});
