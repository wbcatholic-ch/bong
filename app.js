...
function _resetCoverBackTrap(reason){
  /* V4-71: 커버 trap 재설정도 js/back-controller.js의 공통 armCoverBackTrap을 우선 사용한다.
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V4-71';
  }else if(!restore){
...
function closeDioceseView(){
  ...
  /*
     V4-71: 관구·교구는 커버에서 여는 독립 iframe 화면이므로,
     ...
     여기서는 기존 관구·교구 레이어만 닫고 커버 상태와 뒤로가기 대기값만 직접 정상화한다.
  */
  ...
  try{ if(typeof _clearCoverExitArmed === 'function') _clearCoverExitArmed(); }catch(e){ console.warn('[가톨릭길동무]', e); }
  try{
    if(typeof _resetCoverBackTrap === 'function') _resetCoverBackTrap('diocese-close');
    else if(typeof _ensureCoverBackTrap === 'function') _ensureCoverBackTrap('diocese-close');
    else if(typeof window._oaiArmCoverBackTrap === 'function') window._oaiArmCoverBackTrap('diocese-close');
  }catch(e){ console.warn('[가톨릭길동무]', e); }
...
const _PARISH_ASSET_VERSION='V4-71';
...
const _PRAYER_ASSET_VERSION='V4-71';
...
const _RETREAT_ASSET_VERSION='V4-71';
...
const _SHRINE_ASSET_VERSION='V4-71';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
