...
function oaiReleaseStabilityVeil(){
  try{
    ...
    if(reason === 'external-leave'){
      ...
      // V3-4: 외부사이트가 실제로 열려 앱이 hidden/pagehide 상태가 된 경우에는
...
function oaiPrepareRefreshVeil(reason, duration, carryDuration, showBeforeNavigation, beforeNavigationHold, carryToNextDocument){
  try{
    ...
    var now = Date.now ? Date.now() : new Date().getTime();
    /*
       V3-4: 수동 짧은/긴 새로고침은 OK 직후 현재 문서 보호막을 먼저 보여 주되,
...
function oaiHasExternalReturnPending(){
  try{
    ...
    // V3-4: 실제로 앱이 hidden/pagehide 된 적이 있을 때만 "외부사이트에서 복귀"로 본다.
...
function applyExternalReturnStabilize(){
  ...
  // V3-4: 실제 pagehide/hidden이 확인되지 않은 "열기 시도 중" 상태는 복귀로 오판하지 않는다.
  try{
...
function _resetCoverBackTrap(reason){
  /* V3-4: 커버 trap 재설정도 patches.js의 공통 armCoverBackTrap을 우선 사용한다.
...
async function _runClearAppFilesCacheCompletely(){
  ...
  // V3-4: 긴 새로고침은 현재 문서 보호막 하나만 사용한다.
...
// V3-4: 커버 전용 주요 기능 안내. 자동 배너는 제거하고, 사용자가 주요기능 버튼을 누를 때만 상세 안내를 연다.
(function(){
  'use strict';
...
function openPrayerBook(opts){
  ...
  // V3-4: restore 변수 미정의 오류 방지. 주요기도문 초기화가 중간에 끊기면
...
function normalizeCatholicExternalUrl(url){
  ...
  try{
    ...
    // V3-4: 원주·인천교구 대표 홈페이지는 공식 등록 주소가 HTTP이므로
...
function openDioceseExternal(url, state){
  ...
  // V3-4: iframe에서 온 교구 홈페이지 클릭은 지연 setTimeout 없이 즉시 top 페이지를 이동한다.
...
function restoreDioceseExternalState(opts){
  ...
  try{
    ...
    // V3-4 stable: frame.contentWindow가 있다는 이유만으로 '살아 있다'고 판단하면 안 된다.
...
// V3-4: 성당 데이터를 교구별 parishes-*.js 파일로 실제 분리한다.
...
function oaiEnterView(el){
  ...
  try{
    ...
    // V3-4: 카테고리 진입은 화면 자체를 fade하지 않고, 완성된 화면 위의
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  if(!window._noAutoNearby){
    // V3-4: 성당 첫 진입도 기존 기준대로 내주변 탭을 먼저 연다.
...
function _buildParishDioSystem(){
  ...
  Object.entries(_DIO_CFG).forEach(([code,cfg])=>{
    // V3-4: 군종교구는 데이터/검색에는 남기되 지도 위 교구 라벨에서는 제외한다.
    if(code==='ML') return;
...
function renderList(){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V3-4: 성당찾기는 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
...
function filterModal(q){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V3-4: 성당 길찾기 검색도 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
      matchAll = nameNorm.startsWith(nq) || addrNorm.includes(nq);
    } else {
      const tokens=q.trim().split(/\s+/);
      matchAll=tokens.length>=2
