...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V6-143-COVER-TRAP-CHECK: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V6-143-COVER-TRAP-CHECK';
...
/* V6-143-COVER-TRAP-CHECK: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V6-143-COVER-TRAP-CHECK: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V6-143-COVER-TRAP-CHECK: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V6-143-COVER-TRAP-CHECK: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V6-143-COVER-TRAP-CHECK: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V6-143-COVER-TRAP-CHECK: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V6-143-COVER-TRAP-CHECK: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V6-143-COVER-TRAP-CHECK';
...
const _PRAYER_ASSET_VERSION='V6-143-COVER-TRAP-CHECK';
...
const _RETREAT_ASSET_VERSION='V6-143-COVER-TRAP-CHECK';
...
const _SHRINE_ASSET_VERSION='V6-143-COVER-TRAP-CHECK';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
