...
try{
  if(!window.__OAI_SHRINE_VISIT_POPSTATE_BOUND__){
    ...
    window.addEventListener('popstate', function(e){
      if(typeof _isCoreExternalViewOpen==='function' && _isCoreExternalViewOpen()){
        if(e){ try{ e.preventDefault(); e.stopImmediatePropagation(); }catch(_e){} }
        _closeCoreExternalView({fromPopstate:true});
        return;
      }else if(window.__OAI_CORE_EXTERNAL_VIEW_CLOSING_BY_CODE__){
        if(e){ try{ e.preventDefault(); e.stopImmediatePropagation(); }catch(_e){} }
        window.__OAI_CORE_EXTERNAL_VIEW_CLOSING_BY_CODE__=false;
        return;
      }
      if(e && e.state && e.state.oaiExternalReturn){
        try{ e.preventDefault(); e.stopImmediatePropagation(); }catch(_e){}
        try{ if(typeof restoreCoreReturnStateFromExternalGuard==='function') restoreCoreReturnStateFromExternalGuard(); }catch(_e){}
        return;
      }
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V6-90';
...
const CORE_RETURN_BACKUP_KEY='catholic_core_return_backup_v1';
function saveCoreReturnState(extra){
  ...
  try{
    const payload=JSON.stringify(Object.assign(state, extra||{}));
    sessionStorage.setItem(CORE_RETURN_KEY, payload);
    try{ localStorage.setItem(CORE_RETURN_BACKUP_KEY, payload); }catch(_e){}
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V6-90: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V6-90: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{
    ...
    sessionStorage.setItem('oai_external_return_token', token);
    try{ localStorage.setItem('oai_external_return_token', token); }catch(_e){}
    var base=location.href.split('#')[0];
    history.pushState({oaiExternalReturn:true, token:token, kind:kind||'external'}, '', base+'#external-return-'+token);
  }catch(e){ console.warn("[가톨릭길동무]", e); }
  setTimeout(function(){
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  const nav=$('missa-faith-nav');
  if(!view || !frame){
    /* 최후의 예비 경로: 매일미사/성가용 내부 뷰가 없을 때만 사용 */
  ...
  try{ if(typeof _ensureAppBackTrap==='function') _ensureAppBackTrap('shrine-external'); }catch(e){ console.warn('[가톨릭길동무]', e); }
  try{
    view.dataset.faithCurrent='shrineExternal';
    view.dataset.externalReturnSource='shrine';
  }catch(_e){}
  view.classList.add('open','shrine-external-mode');
  if(title){
    const src=extra && extra.source ? String(extra.source) : '';
    title.textContent = src.indexOf('homepage')>=0 ? '홈페이지' : '성지 상세페이지';
  }
  if(nav){ nav.innerHTML=''; nav.style.display='none'; }
  try{ if(typeof oaiEnterView==='function') oaiEnterView(view); }catch(e){ console.warn('[가톨릭길동무]', e); }
  _replaceFaithFrameWith(url);
}
...
function restoreCoreReturnState(){
  ...
  if(!raw){ try{ raw=localStorage.getItem(CORE_RETURN_BACKUP_KEY); }catch(e){ console.warn("[가톨릭길동무]", e); } }
  if(!raw) return false;
  let state=null;
  ...
  try{ localStorage.removeItem(CORE_RETURN_BACKUP_KEY); }catch(e){ console.warn("[가톨릭길동무]", e); }
  ...
  setTimeout(()=>{
    _restoreMapMarkers();
    if(state.fromStampBook && _mode==='shrine' && typeof _openShrineVisitCardsModal==='function'){
      try{ _openShrineVisitCardsModal(); }catch(e){ console.warn("[가톨릭길동무]", e); }
      if(Number.isInteger(state.infoIdx) && state.infoIdx>=0 && typeof _openShrineVisitDetail==='function'){
        setTimeout(function(){ try{ _openShrineVisitDetail(state.infoIdx); }catch(e){ console.warn("[가톨릭길동무]", e); } }, 90);
      }
      setTimeout(()=>{ document.documentElement.classList.remove('oai-returning'); }, 620);
      return;
    }
...
function restoreCoreReturnStateFromExternalGuard(){
  try{
    const hasHash=!!(location.hash && location.hash.indexOf('#external-return-')===0);
    let hasBackup=false;
    try{ hasBackup=!!(sessionStorage.getItem(CORE_RETURN_KEY)||localStorage.getItem(CORE_RETURN_BACKUP_KEY)); }catch(_e){}
    if(hasHash || hasBackup){
      if(hasHash) history.replaceState(null, '', location.href.split('#')[0]);
      try{ sessionStorage.removeItem('oai_external_return_token'); }catch(_e){}
      try{ localStorage.removeItem('oai_external_return_token'); }catch(_e){}
      setTimeout(function(){ try{ restoreCoreReturnState(); }catch(e){ console.warn("[가톨릭길동무]", e); } }, 0);
      return true;
    }
  }catch(e){ console.warn("[가톨릭길동무]", e); }
  return false;
...
window.addEventListener('popstate', function(e){
  try{
    if(e && e.state && e.state.oaiExternalReturn){
      try{ e.preventDefault(); e.stopImmediatePropagation(); }catch(_e){}
      restoreCoreReturnStateFromExternalGuard();
    }
  }catch(err){ console.warn("[가톨릭길동무]", err); }
}, true);
window.addEventListener('hashchange', function(){ restoreCoreReturnStateFromExternalGuard(); }, true);
window.addEventListener('focus', function(){ setTimeout(restoreCoreReturnStateFromExternalGuard, 0); }, true);
document.addEventListener('visibilitychange', function(){ if(!document.hidden) setTimeout(restoreCoreReturnStateFromExternalGuard, 0); }, true);
...
const _PARISH_ASSET_VERSION='V6-90';
...
const _PRAYER_ASSET_VERSION='V6-90';
...
const _RETREAT_ASSET_VERSION='V6-90';
...
const _SHRINE_ASSET_VERSION='V6-90';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
