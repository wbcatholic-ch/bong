...
function hideCoverAndRun(callback) {
  try{
    document.querySelectorAll('.module-view.open,#prayer-view.open,#diocese-view.open,#missa-view.open').forEach(function(v){
      if(typeof window.oaiCloseModuleView === 'function' && v.id !== 'missa-view'){
        window.oaiCloseModuleView(v);
      } else {
...
function _forceCoverAfterPrayerQuickPopup(){
  ...
  try{
    ...
    document.querySelectorAll('.module-view.open,#diocese-view.open,#missa-view.open').forEach(function(v){
      if(typeof window.oaiCloseModuleView === 'function' && v.id !== 'missa-view'){
        window.oaiCloseModuleView(v);
      } else {
        v.classList.remove('open');
...
/* --sb 측정 IIFE 제거: JS로 뒤늦게 세팅하면 초기 0px → 실제값 변화 시
   커버 하단 요소들이 위로 튕기는 문제가 있었음.
   --sb는 :root에서 0px로 초기화, 실제 safe-area는 CSS env()로 직접 참조. */
(function(){
  const c=$('cv-stars');
  if(!c) return;
  for(let i=0;i<22;i++){
...
function oaiEnterView(el){
  ...
  try{
    ...
    el.classList.remove('oai-enter-ready','oai-enter-show','oai-prepaint-view','oai-view-closing');
...
/* 모듈뷰 부드러운 닫기: .oai-view-closing → opacity 0 transition → open 제거
   oai-returning 상태(히스토리 복귀)에서는 즉시 제거해 복귀 깜빡임 방지 */
window.oaiCloseModuleView = function(el, onDone){
  if(!el) return;
  try{
    if(document.documentElement.classList.contains('oai-returning')){
      el.classList.remove('open','oai-view-closing');
      if(typeof onDone === 'function') onDone();
      return;
    }
    el.classList.add('oai-view-closing');
    var done = false;
    function finish(){
      if(done) return; done = true;
      try{ el.classList.remove('open','oai-view-closing'); }catch(e){ console.warn("[가톨릭길동무]", e); }
      if(typeof onDone === 'function') onDone();
    }
    el.addEventListener('transitionend', finish, {once:true});
    setTimeout(finish, 260); // transition 0.18s + 여유
  }catch(e){
    el.classList.remove('open','oai-view-closing');
    if(typeof onDone === 'function') onDone();
  }
