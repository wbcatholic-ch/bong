...
function _applyShrineVisitMapFilter(){
  ...
  if(_mode==='shrine' && _map){
    ...
    if(_shrineVisitMapFilter==='visited'||_shrineVisitMapFilter==='unvisited'){
      try{ setTimeout(function(){ _fitShrineVisitMapFilterBounds(); }, 40); }catch(e){ console.warn('[가톨릭길동무]', e); }
    }
  }
}
...
function _ensureShrineVisitCardsModal(){
  ...
  modal.addEventListener('click', function(e){
    ...
    if(statRow){
      ...
      _renderShrineVisitCardsModal();
      setTimeout(function(){ _scrollShrineVisitDioceseTabIntoView(dioVal,'smooth'); _scrollShrineVisitExpandedStatsIntoView(); }, 80);
      setTimeout(function(){ _scrollShrineVisitDioceseTabIntoView(dioVal,'smooth'); _scrollShrineVisitExpandedStatsIntoView(); }, 260);
      return;
    }
...
function _scrollShrineVisitExpandedStatsIntoView(){
  try{
    ...
    const target=document.querySelector('.shrine-visit-stat-items');
...
function _renderShrineVisitCardsModal(){
  ...
  const active=(rawTab==='unvisited'||rawTab==='stats')?rawTab:'visited';
  ...
  if(modal){
    modal.querySelectorAll('[data-shrine-visit-cards-tab]').forEach(function(btn){
      ...
      btn.textContent=(val==='visited')?'순례한 성지':(val==='unvisited'?'미방문 성지':'통계');
    });
  }
