...
let _shrineVisitCardsDiocese = '전체';
...
function _getShrineVisitDioceses(){
  const set=new Set();
  SHRINES.forEach(function(item){ if(item&&item.diocese) set.add(String(item.diocese)); });
  return ['전체'].concat(Array.from(set).sort(function(a,b){ return a.localeCompare(b,'ko'); }));
}
function _filterShrineVisitEntriesByDiocese(entries){
  if(!_shrineVisitCardsDiocese || _shrineVisitCardsDiocese==='전체') return entries;
...
function _renderShrineVisitDioceseTabs(visitedCount,totalCount){
  ...
  if(!wrap) return;
  const dioceses=_getShrineVisitDioceses();
  wrap.innerHTML=dioceses.map(function(d){
    const selected=String(d)===String(_shrineVisitCardsDiocese||'전체');
    return '<button type="button" class="'+(selected?'active':'')+'" data-shrine-visit-diocese="'+_visitHtmlEsc(d)+'">'+_visitHtmlEsc(d)+'</button>';
  }).join('');
}
...
function _ensureShrineVisitCardsModal(){
  ...
  modal.innerHTML='<div class="shrine-visit-cards-backdrop" data-shrine-cards-close="1"></div><div class="shrine-visit-cards-panel" role="dialog" aria-modal="true" aria-label="나의 순례기록"><div class="shrine-visit-cards-head"><button type="button" id="shrine-visit-cards-back" class="shrine-visit-cards-back" aria-label="뒤로">‹</button><div class="shrine-visit-cards-title">순례 스탬프북</div><button type="button" id="shrine-visit-cards-x" class="shrine-visit-cards-x" aria-label="닫기">×</button></div><div class="shrine-visit-cards-tabs" role="tablist" aria-label="순례기록 분류"><button type="button" data-shrine-visit-cards-tab="visited">순례한 성지</button><button type="button" data-shrine-visit-cards-tab="unvisited">미방문 성지</button></div><div id="shrine-visit-cards-diocese" class="shrine-visit-cards-diocese" aria-label="교구 선택"></div><div id="shrine-visit-cards-stats" class="shrine-visit-cards-stats"></div><div id="shrine-visit-cards-body" class="shrine-visit-cards-body"></div></div>';
  ...
  modal.addEventListener('click', function(e){
    ...
    if(dio){
      e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation();
      _shrineVisitCardsDiocese=dio.getAttribute('data-shrine-visit-diocese')||'전체';
      _renderShrineVisitCardsModal();
      return;
    }
    ...
    _closeShrineVisitCardsModal();
    setTimeout(function(){ _openShrineVisitDetail(idx); }, 520);
  }, true);
  return modal;
}
...
function _ensureShrineVisitDetailView(){
  ...
  view.addEventListener('click', function(e){
    ...
    if(hp){
      ...
      if(url) openCoreExternalUrl(url,{source:'pilgrim-detail-homepage'});
      return;
    }
    ...
    if(guide){
      ...
      if(url) openCoreExternalUrl(url,{source:'pilgrim-detail-guide'});
    }
  }, true);
  return view;
}
...
function _renderShrineVisitDetail(idx){
  ...
  const hpUrl=item.hp?normalizeCatholicExternalUrl(item.hp):'';
  ...
  body.innerHTML='<section class="shrine-visit-detail-hero"><div class="shrine-visit-detail-kicker">순례 기록</div><div class="shrine-visit-detail-count">순례 '+count+'회</div><div class="shrine-visit-detail-recent">최근 순례일 '+_visitHtmlEsc(recent)+'</div><div class="shrine-visit-detail-date-title">순례 날짜</div><div class="shrine-visit-detail-date-list">'+dateHtml+'</div><button type="button" class="shrine-visit-detail-add" data-shrine-detail-add="'+idx+'">순례 날짜 추가</button></section><section class="shrine-visit-detail-info"><div class="shrine-visit-detail-section-title">성지 정보</div><div class="shrine-visit-detail-name">'+_visitHtmlEsc(item.name||'')+'</div><div class="shrine-visit-detail-row"><span>교구</span><strong>'+_visitHtmlEsc(item.diocese||'—')+'</strong></div><div class="shrine-visit-detail-row"><span>주소</span><strong>'+_visitHtmlEsc(item.addr||'—')+'</strong></div><div class="shrine-visit-detail-row"><span>전화</span><strong>'+telText+'</strong></div><div class="shrine-visit-detail-actions">'+telBtn+hpBtn+guideBtn+'</div></section>';
}
...
function _renderShrineVisitCardsModal(){
  ...
  if(stats) stats.innerHTML='<strong>'+visited.length+'</strong><span>/ '+total+'곳 순례</span><em>'+_visitHtmlEsc(_shrineVisitCardsDiocese||'전체')+'</em>';
  const active=(_shrineVisitCardsTab==='unvisited')?'unvisited':'visited';
  const modal=document.getElementById('shrine-visit-cards-modal');
  if(modal){
    modal.querySelectorAll('[data-shrine-visit-cards-tab]').forEach(function(btn){
      ...
      btn.textContent=(val==='visited'?'순례한 성지 ':'미방문 성지 ')+(val==='visited'?visited.length:unvisited.length);
    });
  }
  const entries=(active==='unvisited')?unvisited:visited;
  body.classList.toggle('unvisited', active==='unvisited');
  if(!entries.length){
    body.innerHTML='<div class="shrine-visit-cards-empty">'+(active==='visited'?'아직 순례등록한 성지가 없습니다.':'모든 성지를 순례했습니다.')+'</div>';
    return;
  }
  body.innerHTML='<div class="shrine-visit-card-grid">'+entries.map(function(entry){
    ...
    const isVisited=active==='visited';
    const countText=isVisited?('순례 '+entry.count+'회'):'미방문';
    return '<button type="button" class="shrine-visit-card-badge '+(isVisited?'visited':'unvisited')+' '+typeClass+'" data-shrine-visit-card="'+entry.idx+'"><span class="shrine-visit-card-stamp">'+_visitHtmlEsc(typeLabel)+'</span><span class="shrine-visit-card-dio">'+_visitHtmlEsc(item.diocese||'')+'</span><strong>'+_visitHtmlEsc(item.name||'')+'</strong><span class="shrine-visit-card-count">'+_visitHtmlEsc(countText)+'</span></button>';
  }).join('')+'</div>';
}
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V6-50';
...
const _PARISH_ASSET_VERSION='V6-50';
...
const _PRAYER_ASSET_VERSION='V6-50';
...
const _RETREAT_ASSET_VERSION='V6-50';
...
const _SHRINE_ASSET_VERSION='V6-50';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
