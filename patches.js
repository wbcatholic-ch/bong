...
(function(){
  ...
  /* ─────────────────────────────────────────────
     V1-21 기도문 전용 뒤로가기 컨트롤러 — history 단계 분리 제거
  ...
  function armPrayerBackTrap(reason){
    /* 호환용 함수. V1-21부터 기도문 detail/list용 별도 pushState는 만들지 않는다.
  ...
  function settleCoverTrapAfterPrayer(reason){
    // V1-21: 기도문 팝업 → 커버 후에는 이미 공통 컨트롤러가 history.go(1)로 trap을 복원한 상태다.
  ...
  function prayerListToPopupOrCover(reason){
    try{
      ...
      /* V1-21: 기도문 목록 → 빠른메뉴 팝업 복귀는 직접 팝업을 띄우지 않는다.
