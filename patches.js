...
(function(){
  ...
  /* ─────────────────────────────────────────────
     V1-5 기도문 전용 뒤로가기 컨트롤러 — history 단계 분리 제거
  ...
  function armPrayerBackTrap(reason){
    /* 호환용 함수. V1-5부터 기도문 detail/list용 별도 pushState는 만들지 않는다.
  ...
  function prayerListToPopupOrCover(reason){
    try{
      ...
      if(typeof window.openMassQuickMenu === 'function'){
        window.openMassQuickMenu({keepReturn:true, fromPrayerReturn:true});
      }else{
        var mq = prayerPopup();
        if(mq){
